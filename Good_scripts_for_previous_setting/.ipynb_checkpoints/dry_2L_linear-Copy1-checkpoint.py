...
prob_class = 'IVP' 
restart = False
init_pattern = 'evp_max_growth'  # 'evp_max_growth'
...
problem.add_equation("dt(q1) + u1@grad(Q1) + U1@grad(q1) + nu*q1 = 0")
problem.add_equation("dt(q2) + u2@grad(Q2) + U2@grad(q2) + nu*q2 = 0")
...
if prob_class == 'EVP':
    ...
    for m in m_range:
        ...
        for nk in np.arange(kn_zeros*2):
            ...
            tau1_eigen.append(np.copy(tau_psi1['g'].real))
            tau2_eigen.append(np.copy(tau_psi2['g'].real))

            lift_tau1_eigen.append(np.copy( lift(tau_psi1)['g'].real)) 
            lift_tau2_eigen.append(np.copy( lift(tau_psi2)['g'].real)) 
        ...
        tau1_list.append(np.copy(tau1_eigen))
        tau2_list.append(np.copy(tau2_eigen))

        lift_tau1_list.append(np.copy(lift_tau1_eigen))
        lift_tau2_list.append(np.copy(lift_tau2_eigen))
    ...
    tasks.create_dataset('gradq1', data=gradq1_list)
    tasks.create_dataset('gradq2', data=gradq2_list)
    
    tasks.create_dataset('phi', data=phi)
    tasks.create_dataset('r', data=r)
    tasks.create_dataset('tau1', data=tau1_list)
    tasks.create_dataset('tau2', data=tau2_list)

    tasks.create_dataset('lift_tau1', data=lift_tau1_list)
    tasks.create_dataset('lift_tau2', data=lift_tau2_list)
    ...
    # Analysis
    snapshots = solver.evaluator.add_file_handler(snapshots_file, sim_dt=timestep, max_writes=10000, mode=file_handler_mode)
    ...
    snapshots.add_task(tau_psi1, scales=(1, 1), name='tau1')
    snapshots.add_task(tau_psi2, scales=(1, 1), name='tau2')
