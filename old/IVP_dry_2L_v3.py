...
er = dist.VectorField(coords, name='er', bases=disk)
er['g'][1] = 1
...
# U1 = d3.Differentiate(psi1_0, coords['r'])
# U2 = d3.Differentiate(psi2_0, coords['r'])
U1 = r_field
U2 = -r_field
...
#                         "+ (dr(psi1) + U1) * dphi(lap(psi1) - F1 * (psi1 - psi2)) " \
#                         "+ dr( lap(psi1) - F1 * (psi1 - psi2) + Q1 ) * (-dphi(psi1)) + lift(tau_psi1) = 0")
# problem.add_equation("dt(lap(psi2) + F2 * (psi1 - psi2)) "
#                         "+ (dr(psi2) + U2) * dphi(lap(psi2) + F2 * (psi1 - psi2)) " \
#                         "+ dr( lap(psi2) + F2 * (psi1 - psi2) + Q2 ) * (-dphi(psi2)) + lift(tau_psi2) = 0")

problem.add_equation("dt(lap(psi1) - F1 * (psi1 - psi2)) "
                        "+ U1 * dphi(lap(psi1) - F1 * (psi1 - psi2)) " \
                        "+ dr(Q1) * (-dphi(psi1)) + lift(tau_psi1) " \
                        "= (-dr(psi1)) * dphi(lap(psi1) - F1 * (psi1 - psi2))" \
                        "- dr( lap(psi1) - F1 * (psi1 - psi2) ) * (-dphi(psi1))")

problem.add_equation("dt(lap(psi2) + F2 * (psi1 - psi2)) "
                        "+ U2 * dphi(lap(psi2) + F2 * (psi1 - psi2)) " \
                        "+ dr(Q2) * (-dphi(psi2)) + lift(tau_psi2) " \
                        "= (-dr(psi2)) * dphi(lap(psi2) + F2 * (psi1 - psi2))" \
                        "- dr( lap(psi2) + F2 * (psi1 - psi2) ) * (-dphi(psi2))")
