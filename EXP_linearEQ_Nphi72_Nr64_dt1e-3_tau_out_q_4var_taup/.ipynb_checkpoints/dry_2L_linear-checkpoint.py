...
exp_name = os.path.basename(os.path.dirname(__file__))
print(exp_name)
output_dir = f'/net/fs06/d0/linyao/GFD_Polar_vortex/ddloutput/{exp_name}/'
...
if prob_class == 'EVP':
    dtype = np.complex128
    kn_zeros = 1 # the number of zeros of the Bessel function to store
elif prob_class == 'IVP':
    dtype = np.float64
    timestep = 1e-2
    ...
    if init_pattern == 'evp_max_growth':
        ...
        snapshots_file = 'snapshots_evp'
        checkpoint_path = 'checkpoints_evp'
        checkpoint_file = './checkpoints_evp/checkpoints_evp_s100.h5'
    elif init_pattern == 'random':
        snapshots_file = 'snapshots_rdm'
        checkpoint_path = 'checkpoints_rdm'
        checkpoint_file = './checkpoints_rdm/checkpoints_rdm_s100.h5'
...
# Problem
if prob_class == 'EVP':
    problem = d3.EVP([psi1, psi2, tau_psi1, tau_psi2, q1, q2, taup1, taup2], eigenvalue=s, namespace=locals())
elif prob_class == 'IVP':
    problem = d3.IVP([psi1, psi2, tau_psi1, tau_psi2, q1, q2, taup1, taup2], namespace=locals())

problem.add_equation("dt(q1) + u1@grad(Q1) + U1@grad(q1) = 0")
problem.add_equation("dt(q2) + u2@grad(Q2) + U2@grad(q2) = 0")
problem.add_equation("q1 - (lap(psi1) - F * (psi1 - psi2)) + lift(tau_psi1) + taup1 = 0")
problem.add_equation("q2 - (lap(psi2) + F * (psi1 - psi2)) + lift(tau_psi2) + taup2 = 0")
...
if prob_class == 'EVP':
    ...
    for m in m_range:
        ...
        for nk in np.arange(kn_zeros*2):
            ...
            q1_eigen.append(np.copy(q1['g'].real))
            q2_eigen.append(np.copy(q2['g'].real))
        ...
        q1_list.append(np.copy(q1_eigen))
        q2_list.append(np.copy(q2_eigen))
    ...
    tasks.create_dataset('q1', data=q1_list)
    tasks.create_dataset('q2', data=q2_list)
    ...
    if not restart:
        ...
        if init_pattern == 'random':
            ...
            q1.fill_random('g', seed=42, distribution='standard_normal')
            q2.fill_random('g', seed=42, distribution='standard_normal') 
            ...
            with h5py.File(init_pattern_file, 'r') as f:
                ...
                q1.load_from_global_grid_data(f["tasks/q1"][:][m_max_growth-1,0,:,:].real)
                q2.load_from_global_grid_data(f["tasks/q2"][:][m_max_growth-1,0,:,:].real)
        psi1['g'] *= initv_scale
        psi2['g'] *= initv_scale
        q1['g'] *= initv_scale
        q2['g'] *= initv_scale
    ...
    # Analysis
    snapshots = solver.evaluator.add_file_handler(snapshots_file, sim_dt=snapshots_time, max_writes=10000, mode=file_handler_mode)
    ...
    snapshots.add_task(u1, scales=(1, 1), name='u1')
    snapshots.add_task(u2, scales=(1, 1), name='u2')
    snapshots.add_task(tau_psi1, scales=(1, 1), name='tau_psi1')
    snapshots.add_task(tau_psi2, scales=(1, 1), name='tau_psi2')
    ...
    try:
        ...
        while solver.proceed:
            ...
            if (solver.iteration-1) % print_time == 0:
                max_psi1 = np.sqrt(flow.max('ke'))
                logger.info("Iteration=%i, Time=%e, dt=%e, max(ke)=%e" %(solver.iteration, solver.sim_time, timestep, max_psi1))
    except:
