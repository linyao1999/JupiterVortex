...
k = np.array([jn_zeros(mi,3) / R for mi in m_range])
a1 = k**4 + 2*F*k**2
a2 = gamma*(F + k**2)
a3 = k**8 + F**2*(gamma**2 - 4*k**4)
c_mathematica = (a2 + np.sqrt(a3+0j)) / a1
s_mathematica = -1j * m_range[:,None] * c_mathematica
s_dedalus = np.array(evals_list)

plt.figure(figsize=(6,4))
for i in range(3):
    plt.plot(m_range, s_dedalus.real[:,i], '.', color=f'C{i}', label='Re(s) (Dedalus)')
    #plt.plot(m_range, 10*s_dedalus.imag, '.', color='C1', label='10*Im(s) (Dedalus)')
    plt.plot(m_range, s_mathematica.real[:,i], '-', color=f'C{i}', label=f'Re(s) (Mathematica) {i+1}')
